...
new_func = '''export const filterExploreMediaWithGroq = async (results, mediaType) => {
  ...
  const titlesList = results.map((r, i) => ${i + 1}.  (Rating: )).join("\\n");

  const prompt = You are a media safety and quality classifier for an English video app.
Review the following list of  and return ONLY the line numbers of legitimate, English-language, non-anime, non-adult media that have a valid user rating above 0.0.
  ...
  try {
    ...
    if (match) {
      ...
      if (Array.isArray(allowedIndices)) {
        ...
        console.log([Groq AI Explore Filter]: Reduced items from  to );
        return filtered;
      }
    }
  } catch (err) {
...
print("Added filterExploreMediaWithGroq to groqFilter.js")
