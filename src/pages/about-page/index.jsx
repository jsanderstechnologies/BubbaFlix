...
const APP_VERSION = `v${versionData?.versionName || "1.0.3"}`;
const CURRENT_VERSION_CODE = versionData?.versionCode || 4;
...
const AboutPage = () => {
  ...
  const handleCheckForUpdates = async () => {
    ...
    if (remoteData) {
      const remoteVersionCode = remoteData.versionCode || 4;
      const currentVersionCode = CURRENT_VERSION_CODE;

      if (remoteVersionCode > currentVersionCode) {
        setUpdateResult({
          hasUpdate: true,
          remoteVersion: remoteData.versionName || "1.0.3",
